...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN?
            ...
            + 'FN:I like you:v\n' // MENDING LU TOBAT SU!
            + 'ORG:Creator BOT G GUNA;\n' // KASIH CREDITS GUA SU!!!
            + 'TEL;type=CELL;type=VOICE;waid=6289514343919:+62 895-1434-3919\n' // JANGAN KEK BABI SU
            + 'END:VCARD'
prefix = '/'
blocked = []
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Halo @${num.split('@')[0]}\nSelamat datang di group Jangan Lupa INTRO Yang Baru Masuk... JANGAN NUMCHAR TOLOL. *${mdata.subject}*`
				...
				teks = `Selamat beban group berkurang satu, @${num.split('@')[0]} Selamat jalan...`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6289514343919@s.whatsapp.net"] // replace this with your number
			const adminbotnumber = ["6289527539758@s.whatsapp.net"]
			...
			case 'creator':
                 ...
                 client.sendMessage(from, 'itu owner....',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'report':
                    ...
                    client.sendMessage('6289514343919@s.whatsapp.net', options, text, {quoted: mek})
               ...
               case 'infogempa':
			    ...
			    case 'otagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘 BOT G GUNA 〙', members_id, true)
					break
				case 'clearall':
					if (!isOwner) return reply('Kamu siapa?')
